...
function mkdir(affixes, callback) {
  ...
  fs.mkdir(dirPath, 0o700, function(err) {
    if (!err) {
...
function open(affixes, callback) {
  ...
  fs.open(filePath, RDWR_EXCL, 0o600, function(err, fd) {
    if (!err) {
...
function openSync(affixes) {
  var filePath = generateName(affixes, 'f-');
  var fd = fs.openSync(filePath, RDWR_EXCL, 0o600);
  deleteFileOnExit(filePath);
...
function createWriteStream(affixes) {
  ...
  var stream = fs.createWriteStream(filePath, {flags: RDWR_EXCL, mode: 0o600});
  deleteFileOnExit(filePath);
  return stream;
}
